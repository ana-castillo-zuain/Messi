...
valores_messi_pases <- c(
  calc_percentil(df_pases_f, messi_nombre, "pases_x90"),
  calc_percentil(df_pases_f, messi_nombre, "goal_assist_x90"),
  calc_percentil(df_pases_f, messi_nombre, "assist_xpase"),
  calc_percentil(df_pases_f, messi_nombre, "shot_assist_x90"),
  calc_percentil(df_pases_f, messi_nombre, "assist_xshot_assist")
)

# Nombres de los atributos
atributos_pase <- c("Pases Totales", "Asistencias", "Ratio de asistencias", "Asistencias de tiro", "Asistencia por tiro")
...
valores_messi_goles <- c(
  calc_percentil(df_goles_f, messi_nombre, "goals_x90"),
  calc_percentil(df_goles_f, messi_nombre, "shots_x90"),
  calc_percentil(df_goles_f, messi_nombre, "goals_xshot"),
  calc_percentil(df_goles_f, messi_nombre, "xg_x90"),
  calc_percentil(df_goles_f, messi_nombre, "xg_diff_x90"),
  calc_percentil(df_goles_f, messi_nombre, "xg_diff_porc")
)

# Nombres de los atributos
atributos_goles <- c("Goles", "Tiros", "Goles por tiro", "Expected goals", "Diferencia de goles esperados","Goles sobre experado")
...
ui <- fluidPage(
  titlePanel("Weighted KDE Heatmap of Pass Endpoints"),
  sidebarLayout(
    sidebarPanel(
      sliderInput("x0", "Center X0 (start point):", min = 0, max = 100, value = 50),
      sliderInput("y0", "Center Y0 (start point):", min = 0, max = 100, value = 50),
...
server <- function(input, output, session) {
  ...
  output$heatmapPlot <- renderPlot({
    # Calculate weights based on distance of start points to (x0,y0)
    dist_sq <- (pases$start_x - input$x0)^2 + (pases$start_y - input$y0)^2
    sigma <- input$sigma
    weights <- exp(-dist_sq / (2 * sigma^2))
    
    # Prepare data matrix of end points
    end_points <- cbind(pases$end_x, pases$end_y)
    
    # Compute weighted KDE using ks::kde
    kde_res <- kde(x = end_points, w = weights, h = sigma, compute.cont = FALSE)
    
    # Extract grid and density estimate
    df <- expand.grid(x = kde_res$eval.points[[1]], y = kde_res$eval.points[[2]])
    df$z <- as.vector(kde_res$estimate)
    
    # Plot heatmap with ggplot2 + red dot for input start point
    ggplot(df, aes(x = x, y = y, fill = z)) +
      geom_raster(interpolate = TRUE) +
      scale_fill_gradient(low = "green", high = "red") +
      geom_point(aes(x = input$x0, y = input$y0), color = "red", size = 5, shape = 19) +
      coord_fixed() +
      labs(title = "Weighted KDE Heatmap of Pass Endpoints",
           subtitle = paste0("Weighted by distance from start center (", input$x0, ", ", input$y0, ")"),
           x = "end_x", y = "end_y", fill = "Density") +
      theme_minimal()
  })
}
