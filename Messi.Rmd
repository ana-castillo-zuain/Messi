...
### Limpieza de NA y variables redundantes
...
### Variables por tipo de competicion
...
# Analisis exploratorio de partidos y eventos
...
porc_local
```
### Analisis influencia de Messi el promedio de goles
```{r}
...
### Distribucion de tipo de eventos
...
### Partidos jugados con messi por jugador
...
```{r}
formaciones <- eventos %>%
  group_by(tactics_formation) %>%
  summarize(
    n = n()
  ) %>%
  filter(!is.na(tactics_formation)) %>%
  mutate(tactics_formation = as.character(tactics_formation)) %>%
  arrange(desc(n)) 

ggplot(formaciones, aes(x = reorder(tactics_formation, -n), y = n)) + 
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Frecuencia de formaciones",
       x = "Formacion",
       y = "Frecuencia") +
  theme_minimal()
```

# Extraccion de minutos por jugador
...
# Jugadores participes en cada partido
...
g1 <- head(df_pases_messi,10) %>% filter(!is.na(pass_recipient_name)) %>%
  ...
  ggplot(aes(x = pass_recipient_name, y = pases)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  coord_flip() +
...
g2 <- head(df_pases_a_messi,10) %>% filter(!is.na(player_name)) %>%
  ...
  coord_flip() +
  labs(title = "Pases a Messi de otros jugadores",
       x = "Jugador",
       y = "Cantidad de pases",
       fill = "Posición") +
  theme_minimal()
...
xg_partidos
```
### Analisis de correlación y certeza de predicción
```{r}
...
### Funcion para calculos de percentiles
